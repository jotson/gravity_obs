...
func connect_to_twitch(is_reconnect := false):
...
func listen():
	if reconnect:
		prints(Time.get_datetime_string_from_system(), "Resuming subscriptions probably")
		return
...
func data_received() -> void:
	...
	if data:
		...
		if msg == "session_reconnect":
			...
			TwitchEventSub.reconnect(websocket_url)
